...
TIMING_LOG="output_3x3/logs/timing_tracking.csv"
mkdir -p output_3x3/logs
...
for iteration in $(seq 1 $NUM_ITERATIONS); do
    ...
    iter_file="output_3x3/logs/current_iteration.txt"
    if [ -f "$iter_file" ]; then
        actual_iter=$(cat "$iter_file")
        ../venv/bin/python3 evaluate_model.py $actual_iter 2>/dev/null || echo "Evaluation skipped (no previous model to compare)"
    else
        # Fallback: try to detect from latest model
        ../venv/bin/python3 evaluate_model.py 2>/dev/null || echo "Evaluation skipped (no previous model to compare)"
    fi
    EVAL_END=$(date +%s)
...
TIMING_SUMMARY="output_3x3/logs/training_summary.txt"

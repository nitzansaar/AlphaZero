...
def transform_action(action_index, transform_type):
    ...
    Args:
        action_index: Original action index (0-8)
...
def transform_policy(policy, transform_type):
    ...
    Args:
        policy: Original policy distribution (9 values)
...
def augment_data(state_flat, policy, transform_type):
    ...
    Args:
        state_flat: Flat board state (9 values)
        policy: Policy distribution (9 values)
        transform_type: Type of transformation
    ...
    """
    # Reshape to 2D
    board_2d = state_flat.reshape(3, 3)

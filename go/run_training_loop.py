...
def _make_pie(ax, timings, title, colormap):
    ...
    # Sort by time descending so legend reads top-to-bottom by importance
    ...
    # Draw pie with no labels — legend handles readability
    wedges, _ = ax.pie(
        sizes, colors=colors, startangle=90,
        wedgeprops=dict(edgecolor='white', linewidth=1.5),
    )

    # Build legend entries: "function_name — 123.4s (56.7%)"
    legend_labels = []
...
def generate_timing_pie_charts(logdir, output_dir):
    ...
    chart_idx = 0
    if selfplay_timings:
        _make_pie(axes[chart_idx], selfplay_timings, "Self-Play Time Breakdown", plt.cm.Set3)
        chart_idx += 1
    if train_timings:

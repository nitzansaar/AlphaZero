                       ...
                       float *values, float *policies)
{
     ...
     *
     * planes layout: batch × GO_NN_INPUT_PLANES × NUM_POSITIONS (flat).
     * NN expects:    batch × GO_NN_INPUT_PLANES × BOARD_SIZE × BOARD_SIZE.
     * Both shapes represent the same contiguous memory.
     */
    torch::Tensor input = torch::from_blob(
        const_cast<float *>(planes),
        {batch_size, GO_NN_INPUT_PLANES, BOARD_SIZE, BOARD_SIZE},

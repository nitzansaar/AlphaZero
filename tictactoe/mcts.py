...
class Node:
    ...
    def is_leaf_node(self):
        return len(self.children) == 0 # check if the node is a leaf node
...
class MonteCarloTreeSearch:
    ...
    def init_root_node(self):
        ...
        root_node = Node(prior_prob = 0, player = 1, action_index = None) # create a new node for the root node
    ...
    def backup(self, mtc_steps, winner, player, value):
        for node in reversed(mtc_steps):
            ...
            if winner == None:
                value = value # game not over, use nn estimated value
            elif winner == 0:
                value = 0
            else:
            ...
            node.mean_action_value_of_next_state_Q = node.total_action_value_of_next_state_W / node.total_visits_N # update the mean action value of the next state
    ...
    def run_simulation(self, root_node, num_simulations=1600, player=1): # run the simulation for the given number of simulations
        ...
        for _ in range(num_simulations):
            ...
            node = root_node
            while node.is_leaf_node() == False: # keep traversing until we reach a leaf node
                action_index, node = node.select_best_child() #UCB score to select the best child
                backup_steps.append(node) # add the node to the backup steps
            leaf_node = node 
            ...
            if winner == None:
                ...
                action_probs = action_probs * valid_moves
                next_player = leaf_node.player * -1 # switch player
                leaf_node.expand(action_probs=action_probs, player=next_player, parent=leaf_node)
            
            self.backup(backup_steps, winner, player, value) # backup the value to the root node
        return root_node
